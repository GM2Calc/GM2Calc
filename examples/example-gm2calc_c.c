...
void setup(MSSMNoFV_onshell* model) {
   ...
   for (unsigned i = 0; i < 3; i++) {
      for (unsigned k = 0; k < 3; k++) {
         c_mssm_onshell_set_mq2(model,i,k,500*500); /* 2L */
         c_mssm_onshell_set_ml2(model,i,k,500*500); /* 1L(smuon)/2L */
         c_mssm_onshell_set_md2(model,i,k,500*500); /* 2L */
         c_mssm_onshell_set_mu2(model,i,k,500*500); /* 2L */
         c_mssm_onshell_set_me2(model,i,k,500*500); /* 1L(smuon)/2L */
      }
   }
...
int main() {
   ...
   const double amu =
      + c_calculate_amu_1loop(model)
      + c_calculate_amu_2loop(model);
